...
Do not write production UI until this file is `LOCKED`. Fixed Dutch constants (black, white, structural gray, `rounded-none`, overshoot easing) may exist as token stubs. Font pair, accent, grid, rupture, and type math are empty on purpose.
...
Not selected. Choose exactly one of the four grotesk pairings in `design/sources/anti-slop-dutch-design.md` §3.

## Accent

Not selected. Choose exactly one of Signal Red, Cobalt Blue, Circuit Orange.
